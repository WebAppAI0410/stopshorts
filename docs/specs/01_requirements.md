...
### 4.5 FR-005: 習慣化コーチング（起動時介入）

one secアプリを参考にした、アプリ起動時の介入機能。

| ID | 要件 | 状態 | 備考 |
|----|------|------|------|
| FR-005-1 | アプリ起動検知 | 未実装 | DeviceActivity使用、entitlement必須 |
| FR-005-2 | 即座にブロック→StopShorts起動 | 未実装 | ManagedSettings使用 |
| FR-005-3 | 深呼吸アニメーション（5-10秒） | 未実装 | 設定で時間調整可能 |
| FR-005-4 | 「本当に必要？」問いかけ | 未実装 | パーソナライズ可能 |
| FR-005-5 | 開く/やめる/代替行動の選択肢 | 未実装 | - |
| FR-005-6 | 今日やめた回数の表示 | 未実装 | 統計連携 |

#### 介入パターン（ダッシュボードで設定可能）

| パターン | 1回目 | 2回目 | 3回目以降 | 対象 |
|----------|-------|-------|----------|------|
| やさしめ | 閉じるのみ | 閉じる+StopShorts | StopShortsのみ | 初心者 |
| 標準（デフォルト） | 閉じる+StopShorts | StopShortsのみ | StopShortsのみ | 一般 |
| 厳格 | StopShortsのみ | StopShortsのみ | StopShortsのみ | 本気 |

### 4.6 FR-006: アプリ内訓練モード

StopShorts内でショート動画視聴をシミュレーションし、「スワイプ→止まる」を練習する機能。

| ID | 要件 | 状態 | 備考 |
|----|------|------|------|
| FR-006-1 | TikTok風モックUI | 未実装 | 縦スワイプでコンテンツ切替 |
| FR-006-2 | X回スワイプ後に介入 | 未実装 | 3-5回でランダム |
| FR-006-3 | 深呼吸ガイダンス | 未実装 | 3回呼吸カウンター |
| FR-006-4 | 訓練完了画面 | 未実装 | 「この感覚を覚えて」メッセージ |
| FR-006-5 | 訓練記録の保存 | 未実装 | 累計回数、ストリーク |
...
| ランダム介入 | ランダムタイミングで介入 | 予測不能性で効果維持 |
| 目的確認訓練 | 介入時に「何をしたかったか」入力 | メタ認知の強化 |
| 代替行動訓練 | 介入後に代替行動を実際に行う | 習慣置換の練習 |
| 時間制限訓練 | 30秒スワイプ→強制終了 | 時間感覚の養成 |

### 4.7 FR-007: 課金・サブスクリプション

| ID | 要件 | 状態 | 備考 |
|----|------|------|------|
| FR-007-1 | 3日間無料トライアル | 未実装 | 全機能利用可、カード登録不要 |
| FR-007-2 | 30日プラン（¥980） | 未実装 | - |
| FR-007-3 | 90日プラン（¥2,350、20%OFF） | 未実装 | 習慣形成に最適（66日理論） |
| FR-007-4 | 年間プラン（¥5,880、50%OFF） | 未実装 | 月額¥490相当 |
| FR-007-5 | プラン選択画面 | 未実装 | オンボーディング後に表示 |

> **Note**: 有料プラン間で機能差なし。期間が長いほどお得。
...
- `docs/HABIT_COACHING_FEATURE.md` - 習慣化コーチング機能設計書
