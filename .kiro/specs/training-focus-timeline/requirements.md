...
### AC-1: 3つの状態表示
- [x] 完了状態: ✓アイコン + グレーテキスト + コンパクト1行
- [x] アクティブ状態: 大きなカード + グロー効果 + 「学習を始める」ボタン
- [x] ロック状態: 🔒アイコン + グレーテキスト + コンパクト1行 + タップ不可

### AC-2: タイムライン表示
- [x] 左側に縦線（タイムライン）が表示される
- [x] 各ステップにノード（円）が表示される
- [x] 完了ノードは緑色、アクティブノードはプライマリ色、ロックノードはグレー（テーマ対応）

### AC-3: インタラクション
- [x] アクティブカードの「学習を始める」ボタンをタップで学習開始
- [x] 完了タスクをタップで再閲覧可能
- [x] ロックタスクはタップ不可（視覚的にも無効状態）

### AC-4: テーマ対応
- [x] ライトモード/ダークモード両対応（useTheme() + インラインスタイル）
- [x] グロー効果がiOS/Android両方で表示される（shadow + elevation）

### AC-5: 品質
- [x] TypeScriptエラー0
- [x] 既存の機能（記事表示、クイズ、ワークシート）が正常動作
