...
### 2.5 会話履歴UI（NEW - 2026-01-06）
| 項目 | 説明 | 決定事項 |
|------|------|----------|
| メッセージ完全保存 | 各セッションの全メッセージを永続化 | 完全保存 |
| 履歴アイコン | ヘッダーにアイコン追加→タップでドロワー表示 | ヘッダー配置 |
| セッション数上限 | 過去セッションの保存数 | 無制限 |
| 閲覧モード | 過去セッション選択時、入力無効化＋「現在に戻る」バナー | - |
| SessionSummary拡張 | `messages: Message[]`を追加 | 型定義変更 |
...
| 項目 | 説明 | 優先度 | 対象ファイル |
|------|------|--------|-------------|
| トレーニング画面UI改善 | 現状のUIが貧弱。カード、進捗表示、アニメーション等の強化 | P2 | `app/(main)/training/index.tsx`, `[topicId].tsx` |
| トレーニングコンテンツ表示 | 記事/クイズ/ワークシートの表示UIを洗練 | P2 | 同上 |
| トピック完了アニメーション | 達成感を演出するアニメーション追加 | P3 | - |
